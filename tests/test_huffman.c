...
void test_tree_building_and_code_generation(void) {
    ...
    count_frequencies(test_text, frequency_table);
    TreeNode *root = build_huffman_tree(frequency_table);
...
void test_write_and_read_tree(void) {
    const char *test_text = "aaaabbc";
    ...
    TreeNode *reconstructed_root = read_huffman_tree(file);
    fclose(file);
    ...
    free_tree(root, free_huffman_data);
    free_tree(reconstructed_root, free_huffman_data);
}

void test_compression_decompression(void) {
...
void test_compression_decompression_large(void) {
